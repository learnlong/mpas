...
public class IncTable implements Element {
    ...
    public void addCols(ArrayList<Cell> ncols) {
        if (cols == null)
            cols = new ArrayList<Cell>(ncols);
        else
            cols.addAll(ncols);
    }
    ...
    public Table buildTable() {
        ...
        int ncol = 0;
        for (Cell pc : rows.get(0)) {
            ncol += pc.getColspan();
        }
        Table table = null;
		try {
			table = new Table(ncol);
		} catch (BadElementException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
        ...
        for(int i = 0 ; i < rows.size(); i ++ ){        	
        	 ArrayList<Cell> col = rows.get( i );
        	 for( int j = 0; j < col.size(); j ++){ 
        		 Cell cell = col.get( j );        		 
        		 if( i == 0 ){  
        			 if( j == col.size()-1 ){
        				cell.enableBorderSide( Rectangle.BOTTOM  | Rectangle.TOP );
        			 }
        			 else{
        				 cell.enableBorderSide( Rectangle.BOTTOM | Rectangle.RIGHT | Rectangle.TOP );
        			 }
        		 }else if( i == rows.size() -1 ) {
        			 if( j == 0 ){
        				 cell.enableBorderSide( Rectangle.BOTTOM  | Rectangle.RIGHT );
        			 }
        			 else{
