...
public class ReportMCorrosion  extends StructReportBase{
	 ...
	 * @param areaCode
	 */
	public ReportMCorrosion(Document document, ComModelSeries ms, String ssiCode, String ssiName, String areaCode,SMain sMain,ISyBo syBo) {
		super(document);
	...
	public Table getTableCenter() throws Exception{
		...
		List<Object[]> listItemNames = syBo.getItemName("0", ms.getModelSeriesId(), "A");
		List<Object> itemIdList = new ArrayList<Object>();
		if(listItemNames!=null){
			int rows =0;
			List<Object[]> listItemNames1;
			for(Object[] obj : listItemNames){
				listItemNames1 = syBo.getItemName(obj[1].toString(), ms.getModelSeriesId(), "A");
				if(listItemNames1!=null){
					for(Object[] obj1 : listItemNames1){
						rows=syBo.getLevelCount(obj1[1].toString(), ms.getModelSeriesId(), "A").size();
			...
			tac.addCell(setCell("应力腐蚀级号EDR", fontNow, Element.ALIGN_CENTER, Element.ALIGN_CENTER, 1, 2));
			...
			for(int t= 0;t<itemIdList.size();t++){
				levelList = syBo.getLevelCount(itemIdList.get(t).toString(), ms.getModelSeriesId(), "A");
				if(levelList!=null){
			...
			for (Sy sy : syList){
				if(sy.getS1().getIsMetal()==1){
					...
					tac.addCell(setCell(getStr(sy.getS1().getSsiForm()), fontNow));
					int t=0;
					int cou=0;
					for(Object[] obj : listItemNames){
						t++;
						...
						if(listItemNames1!=null){
							for(Object[] obj1 : listItemNames1){
								...
								List<Object[]> levelLists = syBo.getLevelCount(obj1[1].toString(), ms.getModelSeriesId(), "A");
								if(levelLists!=null){
	...
	public int getTaCenterCols(){
		...
		if(listItemNames!=null){
			for(Object[] obj : listItemNames){
				totalCount += syBo.getItemCount(obj[1].toString(), ms.getModelSeriesId(), "A");
			}
		}
	...
	public Table getTableTitle() throws Exception{
		...
		if(isFirst==0){
			taTitle.addCell(setCell("2.2.2   应力腐蚀评级(金属)", fontCnTitle,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1,0, null));
		}
		taTitle.addCell(setCell("           重要结构项目应力腐蚀评级见表5", fontCnNormal,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1, 0, null));
		taTitle.addCell(setCell("表5   重要结构项目应力腐蚀评级表", fontCnNormal,Element.ALIGN_CENTER, Element.ALIGN_CENTER, 8, 1, 0, null));
