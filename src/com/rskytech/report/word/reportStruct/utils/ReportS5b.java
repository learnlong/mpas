...
public class ReportS5b extends StructReportBase{
	...
	public ReportS5b(Document document, ComModelSeries ms, String ssiCode, String ssiName, String areaCode, IS5Bo s5Bo, SMain sMain) {
		super(document);
		...
		public Table getTableCenter() throws Exception{
			...
			List<Object[]> listItemNames = s5Bo.getItemName("0", ms.getModelSeriesId(), "B");
			List<Object> itemIdList = new ArrayList<Object>();
			if(listItemNames!=null){
				int rows =0;
				List<Object[]> listItemNames1;
				for(Object[] obj : listItemNames){
					listItemNames1 = s5Bo.getItemName(obj[1].toString(), ms.getModelSeriesId(), "B");
					if(listItemNames1!=null){
						for(Object[] obj1 : listItemNames1){
							rows=s5Bo.getLevelCount(obj1[1].toString(), ms.getModelSeriesId(), "B").size();
				...
				tac.addCell(setCell("ADR", fontNow, Element.ALIGN_CENTER, Element.ALIGN_CENTER, 1, 2));
				...
				for(int t= 0;t<itemIdList.size();t++){
					levelList = s5Bo.getLevelCount(itemIdList.get(t).toString(), ms.getModelSeriesId(), "B");
					if(levelList!=null){
				...
				for (S5 s5 : s5List){
					if(s5.getS1().getIsMetal()==0){
						...
						tac.addCell(setCell(getStr(s5.getS1().getSsiForm()), fontNow));
						int t=0;
						int cou=0;
						for(Object[] obj : listItemNames){
							t++;
							...
							if(listItemNames1!=null){
								for(Object[] obj1 : listItemNames1){
									...
									List<Object[]> levelLists = s5Bo.getLevelCount(obj1[1].toString(), ms.getModelSeriesId(), "B");
									if(levelLists!=null){
		...
		public Table getTableTitle() throws Exception{
			...
			taTitle.addCell(setCell("2.2.3    偶然性损伤评级（非金属）", fontCnTitle,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1, 0, null));
			taTitle.addCell(setCell("               重要结构项目偶然损伤评级见表6。", fontCnNormal,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1, 0, null));
			taTitle.addCell(setCell("表6   重要结构项目偶然损伤评级表", fontCnNormal,Element.ALIGN_CENTER, Element.ALIGN_CENTER, 8, 1, 0, null));
		...
		public int getTaCenterCols(){
			...
			if(listItemNames!=null){
				for(Object[] obj : listItemNames){
					totalCount += s5Bo.getItemCount(obj[1].toString(), ms.getModelSeriesId(), "B");
				}
			}
