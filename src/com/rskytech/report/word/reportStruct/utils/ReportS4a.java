...
public class ReportS4a extends StructReportBase{
	...
	private IS4Bo s4Bo;
	private SMain sMain;
	public ReportS4a(Document document, ComModelSeries ms, String ssiCode, String ssiName, String areaCode, IS4Bo s4Bo, SMain sMain) {
		super(document);
	...
	public Table getTableCenter() throws Exception{
		...
		List<Object[]> listItemNames = s4Bo.getItemName("0", ms.getModelSeriesId(), "A");
		List<Object> itemIdList = new ArrayList<Object>();
		if(listItemNames!=null){
			int rows =0;
			List<Object[]> listItemNames1;
			for(Object[] obj : listItemNames){
				listItemNames1 = s4Bo.getItemName(obj[1].toString(), ms.getModelSeriesId(), "A");
				if(listItemNames1!=null){
					for(Object[] obj1 : listItemNames1){
						rows=s4Bo.getLevelCount(obj1[1].toString(), ms.getModelSeriesId(), "A").size();
			...
			tac.addCell(setCell("一般环境损伤级号EDR", fontNow, Element.ALIGN_CENTER, Element.ALIGN_CENTER, 1, 2));
			tac.addCell(setCell("EDR因材料的修正值", fontNow, Element.ALIGN_CENTER, Element.ALIGN_CENTER, 1, 2));
			...
			for(int t= 0;t<itemIdList.size();t++){
				levelList = s4Bo.getLevelCount(itemIdList.get(t).toString(), ms.getModelSeriesId(), "A");
				if(levelList!=null){
			...
			Double object2 = null ;
			for (S4 s4 : s4List){
				if(s4.getS1().getIsMetal()==1){
					tac.addCell(setCell(ssiCode, fontNow));
					tac.addCell(setCell(ssiName, fontNow));
					...
					tac.addCell(setCell(getStr(s4.getS1().getSsiForm()), fontNow));
					int t=0;
					int cou=0;
					for(Object[] obj : listItemNames){ 	
						t++;
						...
						if(listItemNames1!=null){
							for(Object[] obj1 : listItemNames1){
								...
								List<Object[]> levelLists = s4Bo.getLevelCount(obj1[1].toString(), ms.getModelSeriesId(), "A");
								if(levelLists!=null){
	...
	public Table getTableTitle() throws Exception{
		...
		taTitle.addCell(setCell("2.2    重要结构项目评级", fontCnTitle,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1,0, null));
		taTitle.addCell(setCell("2.2.1    环境损伤评级（金属）", fontCnTitle,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1, 0, null));
		taTitle.addCell(setCell("               重要结构项目一般环境损伤评级见表4。", fontCnNormal,Element.ALIGN_LEFT, Element.ALIGN_CENTER, 8, 1, 0, null));
		taTitle.addCell(setCell("表4   重要结构项目一般环境损伤评级表", fontCnNormal,Element.ALIGN_CENTER, Element.ALIGN_CENTER, 8, 1, 0, null));
	...
	public int getTaCenterCols(){
		...
		if(listItemNames!=null){
			for(Object[] obj : listItemNames){
				totalCount += s4Bo.getItemCount(obj[1].toString(), ms.getModelSeriesId(), "A");
			}
		}
