...
function coordination(id,type,taskDesc,area,doSave,val,rec,needTransfer,mark){
...
//				closeAction : 'close',
				...
				buttons : [{
					...
					text : "关闭",
					handler : function() {
					/*if(val!=null&&val!=""&&rec!=null&&rec!=""){
						//val.record.reject();
						rec.set(needTransfer, val.originalValue);
					}
					if(mark!=null){
	        						...
	        						if(testCooValue()){
			        	 			...
			        	 			　　　　 success : function(response, action) {
				        	 					var msg = Ext.util.JSON
				        	 							.decode(response.responseText);
				        	 					if (msg.flag == true) {
					        	 						Ext.Msg.alert(commonality_message,commonality_messageSaveMsg);
				        								window.close();
				        								if(doSave!=null&&doSave!=""){								
				        									doSave();
				        								}
...
//	    	 								 anchor:'95%'   
	    ...
	    /* if(doSave!=null){	    	 
	    	 window.addListener("close",doSave);
	     }else{
