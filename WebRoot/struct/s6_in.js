var count=null;
var oldfinalRemark=null
var oldcpcp=null
var s6Id=null;
var oldValue=null;
var waitMsg =null;
var ssiArray=null;
var ssiNumberArray=null;
var oldIsCpcp = "";
var oldConsiderWear = "";
var delId = new Array();
...
var cWidt = document.body.clientWidth;
	ssiArray= Ext.decode(ssiList);
      var intervalFStore=new Ext.data.Store( {
     	    url:contextPath+"/struct/s6/getIntervalRecords.do?inorOut=int",
		 ...
		 var intervalTSelect = new Ext.form.ComboBox({
			...
			var listStore = new Ext.data.Store( {
				proxy : new Ext.data.HttpProxy( {
					url : contextPath
							+ "/struct/s6/getAdEdRecords.do?region=in&ssiId=" + ssiId
				}),
				reader : new Ext.data.JsonReader( {
					root : "s1List",
					fields : [ {
			...
			var resultStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy( {
										url : contextPath
							+ "/struct/s6/getResultRecords.do?region=in&ssiId=" + ssiId
				}),
				reader : new Ext.data.JsonReader( {
					root : "taskList",
					fields : [ {
			...
			var otherStore = new Ext.data.Store( {
				proxy : new Ext.data.HttpProxy( {
					url : contextPath
							+ "/struct/s6/getOtherRecords.do?region=in&ssiId="+ssiId
				}),
				reader : new Ext.data.JsonReader( {
					root : "otherList",
					fields : [ {
			...
			var colM = new Ext.grid.ColumnModel(
					[
							...
							{
								header :"内部/内外部"+ commonality_mustInput,
								width : 120,
								dataIndex : "intOut",
								editor : new Ext.form.ComboBox({
								editable:false,
			...
			var listGrid = new Ext.grid.EditorGridPanel( {
				title :  returnPageTitle('内部AD/ED分析汇总','s6'),
				cm : colM1,
				sm : new Ext.grid.RowSelectionModel( {
					singleSelect : true
				}),
		...
		function afteredit(val){
			...
			if(metal==0){
				...
				if(intervalTrue!=null&&eaTypeTrue!=null){
						if(count==null){
							Ext.Ajax.request({
									async : false,
									url : contextPath + '/struct/s6/getOtherCount.do',
			...
			function otherGridBeforeedit(val){
				...
				/*if(val.field =='occures'&&rec.get('sourceStep')!='FD'&&rec.get('sourceStep')!='S3'){
					val.cancel = true;
				}*/
				oldValue=val.value;
			}
			...
			function test(number){
				if ((isMaintain == '0')||(modifiedList.length==0&&modifiedResult==0&&modifiedOther==0&&subfinalRemark==oldfinalRemark&&subcpcp==oldcpcp
						&&considerWear1==oldConsiderWear&&coverCpcp1==oldIsCpcp)){
					  if(number){
							goNext(number);
							return true;
						}
			...
			nextOrSave = function saveOrUpdate(number) {
				...
				Ext.MessageBox.show({
				    ...
				    fn : function(id){
						...
						if(step[9] == 1 || step[9] == 2){
							successNext(9);
						}else if(step[11] == 1 || step[11] == 2){
							successNext(11);
			...
			function doSave(){
				...
				var jsonResult = [];
				var jsonOther = [];
				var modifiedList = listStore.modified;
				var modifiedResult = resultStore.modified;
				var modifiedOther = otherStore.modified;
