...
Ext.onReady(function(){
		...
		//	alert(syIdArray);
			...
			function save(number){
					...
					waitMsg.show();
					Ext.Ajax.request( {
							url :  contextPath+"/struct/sy/saveSy.do",
							method : 'post',
	...
	function doSetValue(value){
		...
		//=document.getElementsByName("editItem");
		for(var i =0;i<arrayValue.length;i++){
	...
	function doValue(name,value){
		...
		arrayValue[index]=value+"".trim();
		document.getElementById(name).innerHTML=value;
	}
	

	     function showArea(name,beforeClose,maxLength){
	     	
	        var rightWinPanel = new Ext.form.FormPanel({
					baseCls : 'x-plain',
					title:commonality_title_cn,//'中文信息输入 :',
					labelWidth : 1,
					labelAlign : 'right',
					region : 'west',
					width: 500,
					items : [
						...
						vtype:'htmlTag',
							id : 'showTextArea2',
						   value : document.getElementsByName('remark')[0].value,
							height : 175,
							maxLength:2500,
							///readOnly :true,
							anchor : '90%'
				...
				showWin.show();
				//设置焦点
				Ext.getCmp('showTextArea2').focus(false,true);
